...
const handler = async (m, { conn, usedPrefix }) => {
  ...
  const sessionPath = './Radiosession/';
  try {
    if (!existsSync(sessionPath)) {
      return await conn.sendMessage(m.chat, {text: '*La carpeta Radiosession no existe o está vacía.*'}, {quoted: m});
    }
    ...
    if (filesDeleted === 0) {
      await conn.sendMessage(m.chat, {text: '*No se encontró ningún archivo para eliminar en la carpeta Radiosession.*'}, {quoted: m});
    } else {
