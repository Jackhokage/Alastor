...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `*NOMBRE DE LA CANCION FALTANTE, PORFAVOR INGRESE EL TITULO/NOMRE DE LA CANCION*\n\n*EJEMPLO:*\n*${usedPrefix + command} HELL IS FOREVER* `;
  try {
    const yt_play = await search(args.join(' '));
    let additionalText = '';
    if (command === 'play3' || command == 'play') {
      additionalText = 'audio 🔊';
    } else if (command === 'play4' || command == 'playdoc2') {
      additionalText = 'video 🎥';
    }
    const texto1 = `*⌈YOUTUBE⌋*\n
❏ 𐂂 *Titulo:* ${yt_play[0].title}

❏ 𐂂 *Publicado:* ${yt_play[0].ago}

❏ 𐂂 *Duracion:* ${secondString(yt_play[0].duration.seconds)}

❏ 𐂂 *Vistas:* ${`${MilesNumber(yt_play[0].views)}`}

❏ 𐂂 *Autor:* ${yt_play[0].author.name}

❏ 𐂂 *Canal:* ${yt_play[0].author.url}

❏ ꫞ *ID:* ${yt_play[0].videoId}

❏ 𐂂 *Tipo:* ${yt_play[0].type}

❏ 𐂂 *Link:* ${yt_play[0].url}\n
❏ *_Enviando ${additionalText}, aguarde un momento．．．_*`.trim();
    ...
    if (command == 'play4' || command == 'play2') {
      try {
        const qu = '360';
        const q = qu + 'p';
        const v = yt_play[0].url;
        ...
        try {
          ...
          try {
            ...
            await conn.reply(m.chat, '*ERROR, NO FUE POSIBLE DESCARGAR SU VIDEO*', m);
          }
        }
      }
    }
...
// handler.tags = ['downloader']
// handler.command = /^play|playdoc?$/i
// export default handler