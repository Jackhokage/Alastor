...
• 𐂂 𝚃𝙸𝚃𝚄𝙻𝙾: ${title}
• 𐂂 𝙰𝚁𝚃𝙸𝚂𝚃𝙰: ${artists !== undefined ? artists.map((v) => v.name).join(', ') : 'No encontrado'}
• 𐂂 𝙰𝙻𝙱𝚄𝙼: ${album.name || 'No encontrado'}
• 𐂂 𝙶𝙴𝙽𝙴𝚁𝙾: ${genres !== undefined ? genres.map((v) => v.name).join(', ') : 'No encontrado'}
• 𐂂 𝙵𝙴𝙲𝙷𝙰 𝙳𝙴 𝙻𝙰𝙽𝚉𝙰𝙼𝙸𝙴𝙽𝚃𝙾: ${release_date || 'No encontrado'}
`.trim();
